...
def apriori(dataset, minSupport, verbose=False, maxPatternLength=3):
    ...
    print(f"DEBUG: Total unique items in dataset: {len(itemsInDataset)}")
    print(f"DEBUG: Dataset size: {len(dataset)} sequences")
    print(f"DEBUG: Maximum pattern length limit: {maxPatternLength}")
    ...
    k = 1
    while (True):
        if not Overall[k - 1] or k >= maxPatternLength:
            if k >= maxPatternLength:
                print(f"DEBUG: Stopped at level {k} due to maximum pattern length limit")
            break
        # 1. Candidate generation
        candidatesLastLevel = [x[0] for x in Overall[k - 1]]
        print(f"DEBUG: Level {k+1} - Starting with {len(candidatesLastLevel)} frequent sequences from previous level")
        candidatesGenerated = generateCandidates(candidatesLastLevel)
        print(f"DEBUG: Level {k+1} - Generated {len(candidatesGenerated)} candidates")
        
        # Safety check: if too many candidates, warn and break
        if len(candidatesGenerated) > 100000:
            print(f"WARNING: Too many candidates ({len(candidatesGenerated)}). Stopping to prevent infinite loop.")
            break
